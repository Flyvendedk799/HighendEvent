...
- Delete or quarantine demo-data usage as each surface goes live — no dual sources of truth.
- Prefer wiring existing API modules before inventing new ones.
- Every admin screen must create/read/update/delete real tenant-scoped rows.
- Dashboard UI quality is a first-class deliverable, not a coat of paint at the end.
- Ship in vertical slices that a human can click: auth → catalog CRUD → availability → checkout → booking ops → theme → go-live.
- Do not expand marketing pages until the tenant product path is real.
- Tenant isolation is sacred: every query uses `tenantId`; add tests when touching data paths.
- Match legacy behavior (buffers, deposit/remainder, calendar occupancy, delivery quote) via `@rentora/domain` + API — not by resurrecting Jinja.
...
**B1. Grow `@rentora/ui`** — accessible primitives (Radix + Tailwind is fine) with Rentora tokens:
...
**J1. Analytics** — real charts from bookings/payments (Recharts/visx); utilization by product, revenue, conversion funnel basics
**J2. Coupons** — wire `Coupon` model: % or fixed, date window, usage limits; checkout applies + admin CRUD
**J3. Staff invites** — invite by email, accept link, role assignment UI
**J4. Developer features (Scale plan)** — API keys, webhook endpoints outbound on booking events
**J5. i18n** — make en/da switch real (storefront + email); admin can stay English-first initially; currency/locale formatting everywhere via store settings
**J6. Quality bar pass**
...
### Principles
- **Density with breath** — Shopify-like: tight tables, but 24–32px page padding, clear section gaps
- **One primary action per page** (top-right)
- **Objects over dashboards** — products, bookings, customers are the nouns; KPIs serve them
- **Status is sacred** — color + label + allowed transitions; never free-text status edits
- **Money is tabular** — right-aligned, consistent currency, show tax/deposit breakdown
- **Dates are ranges** — always show timezone from store settings
- **Empty > fake** — empty states beat demo numbers every time
...
## 6. Suggested implementation order (PR sequence)
...
- A stranger can sign up, connect Stripe test mode, add 5 products with photos, theme the shop, and receive a paid test booking
- Admin booking detail is good enough that a warehouse staffer could run a Saturday on it
- Storefront product calendar is trusted (stock + blackouts + buffers)
- Two tenants on one platform have zero data bleed
- Demo data is gone from production paths
- Deployed preview/staging proves checkout + email + admin CRUD
